...
- **Chat:**
    - You can chat with Jarvis in a dedicated sidebar (the **chat panel**, opened with the `Toggle chat panel` command), or inside a note. The panel has three modes, cycled with `Shift+Tab` (or by clicking the mode button): **Chat** for plain conversation, **Note** to scope the chat to the currently open note (like the `Chat with Jarvis` command), and **Collection** to ground it in your whole notes collection (like `Chat with your notes`).
