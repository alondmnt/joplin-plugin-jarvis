...
export async function register_settings() {
  ...
  await joplin.settings.registerSettings({
    ...
    'notes_search_candidate_limit': {
      ...
      description: 'Applies to experimental synced database only. Default: 0 (automatic: mobile 320-960, desktop 1024-8192, scales with requested results). How many candidate blocks to consider when searching. Higher = more accurate but slower and uses more memory. Try 500 for faster search, 2000 for better accuracy.',
    },
    'notes_search_min_nprobe': {
      ...
      description: 'Applies to experimental synced database only. Default: 0 (automatic: mobile 12-14, desktop 16-20). How many note clusters to search—the main speed/accuracy knob. Higher = better results but slower. Try 8 for faster search, 24 for maximum accuracy.',
    },
    'notes_search_small_set_nprobe': {
      ...
      description: 'Applies to experimental synced database only. Default: 0 (automatic: 6-10, half of main thoroughness setting). Search thoroughness when you have few notes. Prevents wasting CPU searching empty clusters. Usually leave at 0.',
    },
    'notes_search_max_rows': {
      ...
      description: 'Applies to experimental synced database only. Default: 0 (automatic: mobile 1200-3840, desktop 6000-24000, scales with result pool). Caps how many embeddings are decoded from storage. Decoding is CPU-intensive. Try 800 for faster search, 5000 for better accuracy.',
    },
    'notes_search_time_budget_ms': {
      ...
      description: 'Applies to experimental synced database only. Default: 0 (automatic: mobile 150ms, desktop 350ms). Emergency brake that stops search to prevent UI freezes. Returns partial results if time exceeded. Try 100 for snappier UI, 500 for thorough search.',
    },
