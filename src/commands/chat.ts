...
import { TextEmbeddingModel, TextGenerationModel, type ModelCallOptions } from '../models/models';
...
export async function chat_with_notes_panel(
  ...
  const result = await run_notes_chat_pipeline(prompt_text, model_embed, model_gen, settings, history, false, opts);
  if (!result) {
...
===`;
    const completion = model_gen.clean_completion(await model_gen.chat(composed, false, opts));
    return `${completion}\n\n[${title}](:/${note.id})`;
  } finally {
...
async function run_notes_chat_pipeline(
  ...
  const [prompt, nearest] = await get_chat_prompt_and_notes(
    model_embed, model_gen, settings, prompt_override_for_retrieval, require_note, opts);
  ...
  try {
    ...
    if (settings.notes_decompose_query && !prompt.search && prompt.notes.size === 0 && !prompt.context) {
      ...
      if (source.length > 0) {
        ...
        const sub_queries = await decompose_query(source, model_gen, opts);
        if (settings.notes_debug_mode) {
